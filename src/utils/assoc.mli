...
(** [lookup k assoc] returns the most recent value associated with [k] in [assoc] if it
    exists. *)

val find_if : ('k * 'v -> bool) -> ('k, 'v) t -> ('k * 'v) option
(** [find_if p assoc] returns the first [(k, v)] for which [p] returns [true] if it
    exists. *)
...
(** [update k v assoc] overshadows the previous value for [k] with [v]. If
    [remove k assoc] is used once, the previous value will be associated with [k] again. *)

val replace : 'k -> 'v -> ('k, 'v) t -> ('k, 'v) t
(** [replace k v assoc] replaces the previous value for [k] with [v] if [k] is associated
    with a value in [assoc]. *)
...
(** [map_of_list f lst] transforms each element of [lst] in a pair [(k, v)] with the
    function [f] and builds an association list from it. *)
...
val fold_map : ('acc -> 'v -> 'acc * 'w) -> 'acc -> ('k, 'v) t -> 'acc * ('k, 'w) t
(** [fold_map f state assoc] folds from left to right ON VALUES and also maps at the same
    time. *)

val kfold_map :
  ('acc -> 'k * 'v -> 'acc * ('h * 'w)) -> 'acc -> ('k, 'v) t -> 'acc * ('h, 'w) t
...
(** [values_of assoc] constructs a list of all values in [assoc] in the same order. *)
...
(** [concat assoc1 assoc2] joins two association lists. If keys are repeated, the values
    in [assoc1] overshadow those of [assoc2]. *)
